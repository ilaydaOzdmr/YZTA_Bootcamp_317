...
SCENARIOS = {
    "0_baseline":             ("Baseline (kriz)", lambda r, p: (r, p)),
...
def status_of(trough: float) -> str:
    if trough < 0:
        return "KRIZ"                    # kasa eksiye duser
    if trough < MIN_CASH_BUFFER:
        return "KURTARILDI"              # pozitif ama guvenli tampon alti
    return "GUVENLI"
...
def main() -> None:
    ...
    for key, (label, fn) in SCENARIOS.items():
        ...
        proj = project(r, p, start_balance, future, 1.0)   # normal gecikme faktoru
    ...
    print("=" * 74)
    print(f"{'Senaryo':<42}{'En dusuk kasa':>15}{'Durum':>12}{'Delta':>14}")
    print("-" * 74)
    for x in results:
        d = f"{x['trough'] - base:+,.0f}" if x["key"] != "0_baseline" else "-"
        print(f"{x['label']:<42}{x['trough']:>11,.0f} TL {x['status']:>11}  {d:>13}")
